...
function validateQuestion(q, state) {
  ...
  for (const prev of state.asked) {
    if (S.jaccard(prev, q) >= 0.55) problems.push("near-duplicate of an asked question");
  }
  return problems;
}
...
function buildContext(state, lastAnswer) {
  const g = S.gaps(state);
  const heat = S.heatFrom(state, state.turn);
  return [
    "KNOWN (never ask about any of this):",
    S.knownDigest(state) || "- nothing yet",
    "",
    "GAPS (priority order):",
    g.map((k) => `- ${k}: ${S.CATEGORIES[k].label}`).join("\n") || "- none",
    "",
...
async function nextQuestion(model, state, lastAnswer) {
  const ctx = buildContext(state, lastAnswer);
  let messages = [
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    if (parsed && typeof parsed.question === "string") {
      ...
      if (!problems.length) {
        ...
        return { question: parsed.question.trim(), reflection, target: parsed.target || null, source: "model" };
      }
      messages = [
        ...messages,
  ...
  const g = S.gaps(state);
  const cat = g[0] || "specific";
  return { question: FALLBACKS[cat], reflection: null, target: cat, source: "fallback" };
}
